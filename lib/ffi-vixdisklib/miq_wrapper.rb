...
require 'drb/drb'
require 'sync'
require 'ffi-vixdisklib/api_wrapper'
require 'VimTypes'
require 'log4r'
require 'time'
require 'vmdb-logger'
...
class MiqWrapper
  ...
  def self.connect(connect_parms)
    $vim_log.info "MiqWrapper.connect: #{connect_parms[:server_name]}" if $vim_log
  ...
  def self.__disconnect__(conn_obj)
    $vim_log.info "MiqWrapper.__disconnect__: #{conn_obj.serverName}" if $vim_log
    raise VixDiskLibError, "VixDiskLib is not initialized" unless @initialized
...
class VdlConnection
  ...
  def disconnect
    ...
    @disk_lock.synchronize(:EX) do
      if !@vdl_connection
        ...
        __close_disks__
        MiqWrapper.__disconnect__(self)
        @vdl_connection = nil
        @vddk.running = true
        @vddk.shutdown = true
      end
  ...
  def getDisk(path, flags)
    @disk_lock.synchronize(:EX) do
      ...
      @disks << disk
      nd = MiqWrapper.inc_server_disk_count
      ...
      if nd >= MAX_DISK_WARN && $vim_log
        ...
        MiqWrapper.dumpDisks(@serverName)
      end
      return disk
    end
  end
  ...
  def __close_disk__(diskObj)
    ...
    if !@vdl_connection
      ...
      nd = MiqWrapper.dec_server_disk_count
